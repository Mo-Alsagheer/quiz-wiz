...
import {QuizAttempt, QuizAttemptDocument, AttemptStatus} from 'src/schemas/quiz-attempt.schema';
...
async joinQuiz(
  ...
  if (
    now < quiz.codeValidFrom ||
    now > quiz.codeValidUntil
  ) {
  ...
  if (now > quizEndTime) {
    throw new BadRequestException(
      await this.i18n.translate(
        'common.quiz.timeExceeded',
      ),
    );
  }

  const submittedAttempt =
    await this.quizAttemptModel.findOne({
      quizId: quiz._id,
      learnerId,
...
async submitQuiz(
  ...
  const quiz = await this.quizModel.findById(
    quizId,
  );
  ...
  const timeTaken = Math.floor(
    (now.getTime() -
      attempt.attemptStartTime.getTime()) /
      1000,
  );
